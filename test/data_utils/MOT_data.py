...
class MOT_data(Dataset):
    ...
    def create_sample(self, candidate_box, candidate_label, sequence_boxes=None, sequence_velocities=None, sequence_locations=None, sequence_frame_numbers=None, sequence_occupancy_grids=None,sequence_crops = None):
        ...
        if self.crops:
            ...
            if self.augment:
                ...
                if random.random() < 0.5:
                    image_crops = [tr_F.hflip(image_crop) for image_crop in image_crops]
                    candidate_crop = tr_F.hflip(candidate_crop)
...
def get_loaders(
    ...
    composed = transforms.Compose([ToTensor(crops=crops,
                                            velocities=velocities, occupancy_grids=occupancy_grids,augment=augment)])
    train_dataset = MOT_data(composed,cuhk=cuhk, crops=crops, velocities=velocities, occupancy_grids=occupancy_grids, test_set=False, sequence_length=train_sequence_length, future_seq_len=future_seq_len,data_mot=data_mot,device = device,video_names =video_names)
