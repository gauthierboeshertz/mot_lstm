...
def gen_crops(path ):
    detections_list,gts_list,frames_list = gen_mot2015_npy(path)
    detections_list16,gts_list16,frames_list16 = gen_mot2016_npy(path)
        ...
        for frame_num in gt_by_frame:
            ...
            for idx,gt in enumerate(gt_in_frame):
                ...
                if np.any(iod>0.5):
                    ...
                    img_path = str(int(gt_id)).zfill(3)+'_'+str(int(frame_nbr)).zfill(3)+'.jpg'

                    im_to_save.save(path+'/mot_cropped/'+str(gts_list_idx)+'/'+img_path)
            if frame_num%100 ==0:
...
if __name__ == '__main__':
    ...
    # Network settings
    parser.add_argument('--path', type=str, default='/media/data/gauthier',
